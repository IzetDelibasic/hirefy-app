// - React -
import { Link } from "react-router-dom";
// - Components -
import { CustomButton } from "..";

const LandingPageAction = () => {
  return (
    <div className="flex flex-col items-center justify-center md:flex-row md:justify-center md:w-[30rem] lg:w-full w-[90%] mx-auto md:mx-0">
      <Link to="/register">
        <CustomButton
          className="relative bg-blue-500 text-white font-medium lg:py-[1.2rem] py-[1rem] lg:px-[2.5rem] px-[3.5rem] md:px-[1.5rem] mr-0 mb-[20px] md:mb-0 rounded-[3rem] group z-[1] whitespace-nowrap overflow-hidden text-ellipsis"
          iconClassName="group-hover:text-white ml-[10px]"
          title="Deep In Adventure"
          titleClassName="group-hover:text-white font-subtitle whitespace-nowrap overflow-hidden text-ellipsis"
          href="/register"
        >
          <div className="absolute inset-0 bg-black w-full transform origin-right transition-transform duration-300 group-hover:scale-x-0 z-[-1]"></div>
        </CustomButton>
      </Link>
      <CustomButton
        className="relative bg-blue-500 text-white font-medium lg:py-[1.2rem] py-[1rem] lg:px-[2rem] px-[3.5rem] md:px-[1.5rem] rounded-[3rem] group z-[1] md:ml-[20px] hover:border-opacity-0 hover:border-transparent whitespace-nowrap overflow-hidden text-ellipsis"
        iconClassName=""
        title="Find Opportunity"
        titleClassName="group-hover:text-white font-subtitle whitespace-nowrap overflow-hidden text-ellipsis"
        href="/login"
      >
        <div className="absolute inset-0 bg-black w-full transform origin-right transition-transform duration-300 group-hover:scale-x-0 z-[-1]"></div>
      </CustomButton>
    </div>
  );
};

export default LandingPageAction;
